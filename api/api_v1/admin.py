...
router = APIRouter(dependencies=[Depends(require_admin)], prefix=settings.api.v1.admin,tags=["Admin"])



@router.post("/projects", response_model=ProjectRead)
async def admin_create_project(project: ProjectCreate, db: AsyncSession = Depends(db_helper.session_getter)):
    new_project = await create_project(db, project)
    return new_project


@router.put("/projects/{project_id}", response_model=ProjectRead)
async def admin_update_project(project_id: int, project: ProjectUpdate, db: AsyncSession = Depends(db_helper.session_getter)):
...
async def admin_delete_project(project_id: int, db: AsyncSession = Depends(db_helper.session_getter)):
...
async def admin_delete_image(project_id: int, image_id: int, db: AsyncSession = Depends(db_helper.session_getter)):
...
async def admin_reorder_images(project_id: int, image_orders: Dict[int, int], db: AsyncSession = Depends(db_helper.session_getter)):
    project = await get_project(db, project_id)
    if not project:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found")
